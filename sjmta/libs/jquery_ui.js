...
(function(a) {
    a.widget("ui.draggable", a.ui.mouse, {
        ...
        _mouseDrag: function(d, c) {
            ...
            // console.log("this.positionAbs",this.positionAbs);
        ...
        _convertPositionTo: function(d, c) {
            ...
            var scaleFactor = 1; //Scale functionality based on container scale - Updated by Guru
            if(containerScaleFactor != null)
                scaleFactor = containerScaleFactor;    
            var clickParentTop = this.offset.parent.top / scaleFactor;
            var clickParentLeft = this.offset.parent.left / scaleFactor;

            return {
                top: c.top +
                    this.offset.relative.top * d + clickParentTop * d - (a.browser.safari && a.browser.version < 526 && this.cssPosition == "fixed" ? 0 : (this.cssPosition == "fixed" ? -this.scrollParent.scrollTop() : h ? 0 : e.scrollTop()) * d),
                left: c.left + this.offset.relative.left * d + clickParentLeft * d - (a.browser.safari && a.browser.version < 526 && this.cssPosition == "fixed" ? 0 : (this.cssPosition == "fixed" ? -this.scrollParent.scrollLeft() : h ? 0 : e.scrollLeft()) * d)
            }
        },
        _generatePosition: function(d) {
            ...
            if(containerScaleFactor != null)
                scaleFactor = containerScaleFactor;
            var clickTopVal = this.offset.click.top / scaleFactor;
            var clickLeftVal = this.offset.click.left / scaleFactor;
            var parentLeftVal = this.offset.parent.left / scaleFactor;
            var parentTopVal = this.offset.parent.top / scaleFactor;
            ...
            var c = this.options,
                ...
                h = /(html|body)/i.test(e[0].tagName),
                g = d.pageX / scaleFactor,
                i = d.pageY / scaleFactor;
            if (this.originalPosition) {
                ...
                if (this.containment) {
                    ...
                    if (d.pageX / scaleFactor - clickLeftVal < b[0]) g = b[0] + clickLeftVal;
                    if (d.pageY / scaleFactor - clickTopVal < b[1]) i = b[1] + clickTopVal;
                    if (d.pageX / scaleFactor - clickLeftVal > b[2]) g = b[2] + clickLeftVal;
                    if (d.pageY / scaleFactor - clickTopVal > b[3]) i = b[3] + clickTopVal
                }
                if (c.grid) {
                    i = c.grid[1] ? this.originalPageY + Math.round((i - this.originalPageY) / c.grid[1]) * c.grid[1] : this.originalPageY;
                    i = b ? !(i - clickTopVal < b[1] || i - clickTopVal > b[3]) ? i : !(i - clickTopVal < b[1]) ? i - c.grid[1] : i + c.grid[1] : i;
                    g = c.grid[0] ? this.originalPageX + Math.round((g - this.originalPageX) /
                        c.grid[0]) * c.grid[0] : this.originalPageX;
                    g = b ? !(g - clickLeftVal < b[0] || g - clickLeftVal > b[2]) ? g : !(g - clickLeftVal < b[0]) ? g - c.grid[0] : g + c.grid[0] : g
                }
            }

            return {
                top: i - clickTopVal - this.offset.relative.top - parentTopVal + (a.browser.safari && a.browser.version < 526 && this.cssPosition == "fixed" ? 0 : this.cssPosition == "fixed" ? -this.scrollParent.scrollTop() : h ? 0 : e.scrollTop()),
                left: g - clickLeftVal - this.offset.relative.left - parentLeftVal + (a.browser.safari && a.browser.version <
        ...
        drop: function(d, c) {
            ...
            a.each(a.ui.ddmanager.droppables[d.options.scope] || [], function() {
                if (this.options) {
                    ...
                    //Update offset Position based on container scale factor - Updated By Guru
                    var scaleFactor = 1;
                    if(containerScaleFactor != null)
                        scaleFactor = containerScaleFactor;
                    ...
                    if(this.offset != null)
                    {
                        this.offset.top = this.offset.top / scaleFactor;
                        this.offset.left = this.offset.left / scaleFactor;
                    }
...
(function(a) {
    a.widget("ui.slider", a.ui.mouse, {
        ...
        _mouseCapture: function(d) {
            ...
            //Updated to Scalefactor based on Container scale value - Updated by Guru
            var scaleFactor = 1;
            if(containerScaleFactor != null)
                scaleFactor = containerScaleFactor;

            var valueX = d.pageX / scaleFactor;
            var valueY = d.pageY / scaleFactor;
            ...
            //Guru
            this.elementOffset.top = this.elementOffset.top / scaleFactor;
            this.elementOffset.left = this.elementOffset.left / scaleFactor;
            ...
            //Guru
            var leftVal = c.left / scaleFactor;
            var topVal = c.top / scaleFactor;

            this._clickOffset = !a(d.target).parents().andSelf().is(".ui-slider-handle") ? {
                left: 0,
                top: 0
            } : {
                left: valueX - leftVal - g.width() / 2,
                top: valueY - topVal - g.height() / 2 - (parseInt(g.css("borderTopWidth"), 10) || 0) - (parseInt(g.css("borderBottomWidth"), 10) || 0) + (parseInt(g.css("marginTop"), 10) || 0)
            };
